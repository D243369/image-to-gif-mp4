...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Upload interactions
  // Prevent double-open on mobile/desktop: stop propagation from the real <input>
  fileInput.addEventListener("click", (e) => {
    e.stopPropagation();
  });

  dropzone.addEventListener("click", (e) => {
    // If the click originated from the input itself (or its descendants), do nothing
  ...
  dropzone.addEventListener("keydown", (e) => {
    ...
    // Avoid re-trigger if focus is already on the input
    if (e.target === fileInput || fileInput.contains(e.target)) return;
    e.preventDefault();
    fileInput.click();
  });
  ...
  dropzone.addEventListener("dragover", (e) => {
    e.preventDefault();
    dropzone.style.borderColor = (getComputedStyle(document.documentElement).getPropertyValue("--accent").trim() || "#7c3aed");
  });
